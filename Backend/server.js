...
const TaskState = mongoose.model('TaskState', StateSchema, 'tasks');
...
async function ensureState(userId) {
  let state = await TaskState.findOne({ userId });
  if (!state) state = await TaskState.create({ userId, transcriptHistory: [], tasks: [], doneTasks: [] });
  return state;
}
